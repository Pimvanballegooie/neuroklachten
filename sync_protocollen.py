...
# -*- coding: utf-8 -*-
"""
Sync protocollen vanuit Google Docs — Neuro Netwerk (neuroklachten.net)
...
1. Leest protocollen-config.json:
   {
     "protocollen": [
       {
         "id": "cva-mobiliseren",
         "naam": "Mobiliseren na CVA",
         "zone": "cva-nah",          <- cva-nah | parkinson | ms | neuromusculair | duizeligheid
         "niveaus": {
...
# ══════════════════════════════════════════════════════════════════
# SITE-INSTELLINGEN — Neuro Netwerk
# ══════════════════════════════════════════════════════════════════
SITE_URL     = "https://neuroklachten.net"
SITE_NAAM    = "Neuro Netwerk"
LOGO_BESTAND = "neuroklachten_logo.png"
...
# Vijf aandoeningen (categorie 3 "Neurologie" in Supabase) — geen thema-zone
ZONES = {
    "cva-nah":        "CVA en NAH",
    "parkinson":      "Parkinson",
    "ms":             "MS",
    "neuromusculair": "Neuromusculaire aandoeningen",
    "duizeligheid":   "Duizeligheid en evenwicht",
}
ZONE_ICONS = {
    "cva-nah": "🧠",
    "parkinson": "🖐️",
...
def haal_doc_tekst(doc_id: str) -> str | None:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "NeuroNetwerk-Sync/1.0"})
...
def header_html(root_pad: str = "") -> str:
  ...
  <div class="header-inner">
    <a href="{root_pad}index.html" class="logo">
      ...
      <div class="logo-text"><span>Neuro</span> Netwerk</div>
    </a>
...
def bouw_protocol_pagina(protocol: dict, niveau: str, inhoud_html: str) -> str:
    ...
    if is_patient:
      ...
      ℹ️ Deze uitleg hoort bij een behandelprotocol van het Neuro Netwerk en is geen vervanging
...
window.addEventListener('beforeunload', () => speechSynthesis.cancel());
      ...
      ⚕️ Klinische versie voor aangesloten therapeuten van het Neuro Netwerk. Toepassing vraagt
...
<head>
  ...
  <style>
    ...
    .voorlees-balk {{ display: flex; align-items: center; gap: 12px; background: var(--teal-light); border: 1px solid rgba(46,155,143,0.3); border-radius: 12px; padding: 12px 16px; margin-bottom: 22px; }}
...
def bouw_overzicht(protocollen: list, previews: dict) -> str:
    zone_filters = '<button class="zone-filter actief" data-zone="alle" onclick="filterZone(this)">Alle aandoeningen</button>'
    ...
    if protocollen:
      ...
      <p style="color:var(--text-muted);font-size:0.9rem;max-width:440px;margin:0 auto">
        Onze expertteams werken aan de eerste behandelprotocollen voor CVA/NAH, Parkinson, MS, neuromusculaire aandoeningen en duizeligheid.
...
<head>
  ...
  <meta name="description" content="Behandelprotocollen van het {SITE_NAAM}: CVA & NAH, Parkinson, MS, neuromusculaire aandoeningen en duizeligheid & evenwicht. Elk protocol in een leesbare versie voor patiënten en een klinische versie voor therapeuten." />
  <style>
    {basis_css()}
    .hero {{ background: linear-gradient(160deg, #4CAF7D 0%, #2E9B8F 50%, #2E7DA0 100%); position: relative; color: white; text-align: center; padding: 56px 24px 48px; }}
    ...
    .zoek-input:focus {{ border-color: var(--teal); box-shadow: 0 0 0 3px rgba(46,155,143,0.12); }}
...
<div class="pagina">
  <div class="zoek-rij">
    <input type="text" class="zoek-input" id="zoek-input" placeholder="🔍 Zoek bijv. CVA, Parkinson, duizeligheid..." oninput="filterProtocollen()" />
  </div>
